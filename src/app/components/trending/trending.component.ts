    ...
    @if (trendingShows().length > 0) {
      <div class="mt-14 animate-fade-in">
        <div class="flex items-center justify-between mb-6 border-b border-white/5 pb-2">
          ...
          <a routerLink="/top-series" class="text-sm font-semibold text-blue-400 hover:text-blue-300 transition-colors flex items-center gap-1">
            View all (Top 250)
            ...
            <div (click)="openDetails.emit(show)" class="flex-none w-60 glass rounded-2xl p-3 flex flex-col hover-lift cursor-pointer group snap-start">
              <div class="relative aspect-[2/3] overflow-hidden rounded-xl mb-3 shadow-md">
                ...
                @if (state.isWatched(show.id)) {
                  <div class="absolute top-2 left-2 px-2 py-0.5 bg-emerald-500/90 text-white rounded-md flex items-center gap-1 text-[10px] font-bold border border-emerald-400/20 backdrop-blur-sm shadow-lg z-20 animate-fade-in">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                    Watched
                  </div>
                } @else if (state.isInPending(show.id)) {
                  <div class="absolute top-2 left-2 px-2 py-0.5 bg-violet-600/90 text-white rounded-md flex items-center gap-1 text-[10px] font-bold border border-violet-500/20 backdrop-blur-sm shadow-lg z-20 animate-fade-in">
...
export class TrendingComponent implements OnInit {
  ...
  /** List of top-rated shows to display in the trending carousel. */
  trendingShows = signal<TVShow[]>([]);

  /** Emits when the user clicks a show card to view its details. */
  ...
  /** Fetches the top 10 rated shows on component initialization. */
  ngOnInit(): void {
